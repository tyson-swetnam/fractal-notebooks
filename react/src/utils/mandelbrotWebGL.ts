interface MandelbrotWebGLParams {
  centerX: number;
  centerY: number;
  zoom: number;
...
export class MandelbrotWebGLGenerator {
  private canvas: HTMLCanvasElement;
  private gl: WebGLRenderingContext;
  private program!: WebGLProgram;
  private params: MandelbrotWebGLParams;
  private uniformLocations: { [key: string]: WebGLUniformLocation | null } = {};

  constructor(canvas: HTMLCanvasElement, params: MandelbrotWebGLParams) {
    this.canvas = canvas;
    this.params = params;
    ...
    const fragmentShaderSource = `
      ...
      vec3 getColor(int iterations, int maxIter) {
        ...
        if (u_colorScheme == 0) { // classic
          float hue = t * 6.28318; // 2 * PI for full color wheel
          return vec3(
            0.5 + 0.5 * cos(hue),
          ...
          float hue = t * 12.56636; // 4 * PI for rapid color cycling
          return vec3(
            0.5 + 0.5 * sin(hue),
  ...
  public updateParams(params: MandelbrotWebGLParams): void {
    this.params = params;
  }
